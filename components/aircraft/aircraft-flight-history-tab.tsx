...
export function AircraftFlightHistoryTab({ flightLogs }: AircraftFlightHistoryTabProps) {
  ...
  const flights = flightLogs.filter((flight) => {
    ...
    const dateToCheck = flight.booking?.end_time || flight.actual_end || flight.created_at
          ...
          <CardContent className="p-0">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {flights.map((flight) => {
                    const flightDate = flight.booking?.end_time || flight.actual_end || flight.created_at
