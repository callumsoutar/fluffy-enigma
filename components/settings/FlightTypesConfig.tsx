...
import { Plus, Edit, Archive, AlertCircle, Plane } from "lucide-react";
import { cn } from "@/lib/utils";
import type { FlightType, InstructionType, FlightTypeFormData } from "@/lib/types/flight-types";
...
export function FlightTypesConfig() {
  ...
  const handleDelete = async (id: string) => {
    if (
      !confirm(
        "Are you sure you want to deactivate this flight type? This will make it unavailable for new bookings."
      )
    ) {
      return;
    }
    ...
    try {
      setError(null);
      const response = await fetch(`/api/flight-types?id=${id}`, {
        method: "DELETE",
      });

      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || "Failed to deactivate flight type");
      }
  ...
  const instructionTypeOptions: { value: InstructionType; label: string }[] = [
    ...
    return (
      <div className="space-y-6 max-w-4xl">
        <div className="flex items-center justify-center py-12">
          <div className="text-slate-500">Loading flight types...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 max-w-6xl">
      ...
      <p className="text-sm text-slate-600">
        Configure flight types and instruction categories for your operations.
      </p>
      ...
      <div className="flex justify-between items-center">
        <div className="text-sm text-slate-500">
          {flightTypes.length} flight {flightTypes.length === 1 ? "type" : "types"}
        </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <DialogDescription className="mt-0.5 text-sm text-slate-500">
                      Create a new flight type category. Required fields are marked with{" "}
                    ...
                    <div className="grid gap-5">
                      <div>
                        ...
                        <Input
                          ...
                          placeholder="e.g., Navigation Training"
                      ...
                      <div className="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50/60 p-3">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-[11px] text-slate-600 mt-1 leading-snug">
                            Whether this flight type is available for new bookings.
                          </p>
                        </div>
                      </div>
                    </div>
      ...
      {flightTypes.length === 0 ? (
        <div className="text-center py-16 bg-slate-50 rounded-xl border border-dashed border-slate-300">
          ...
          <p className="text-slate-900 font-semibold mb-2">No flight types configured</p>
          <p className="text-sm text-slate-500 mb-4">
            Click &quot;Add Flight Type&quot; to get started.
          </p>
        </div>
      ) : (
          ...
          <Table>
            ...
            <TableBody>
              {flightTypes.map((flightType) => (
                <TableRow key={flightType.id} className="hover:bg-slate-50">
                  <TableCell className="font-medium text-slate-900">
                    <div>
                      <div>{flightType.name}</div>
                  ...
                  <TableCell className="text-slate-600">
                    {flightType.instruction_type ? (
                      <span className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium bg-indigo-50 text-indigo-700 border border-indigo-200 capitalize">
                        {flightType.instruction_type}
                      </span>
                    ) : (
            ...
            <DialogHeader className="px-6 pt-[calc(1.5rem+env(safe-area-inset-top))] pb-4 text-left sm:pt-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <DialogDescription className="mt-0.5 text-sm text-slate-500">
                    Update flight type category details. Required fields are marked with{" "}
                    <span className="text-destructive">*</span>.
                  </DialogDescription>
                </div>
              </div>
