...
type TooltipContentVariant = "default" | "card"
...
function TooltipContent({
  ...
  const contentClasses =
    variant === "card"
      ? "bg-popover text-popover-foreground border shadow-md rounded-lg overflow-hidden"
      : "bg-foreground text-background rounded-md px-3 py-1.5 text-xs text-balance"

  const arrowClasses =
    variant === "card"
      ? "bg-popover fill-popover z-50 size-2.5 translate-y-[calc(-50%_-_2px)] rotate-45 rounded-[2px] border border-border"
