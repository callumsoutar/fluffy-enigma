...
export function DashboardBookings() {
  ...
  return (
    <div className="px-4 lg:px-6 pb-10 space-y-10">
      ...
      {(isLoadingRequested || requestedBookings.length > 0) && (
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <TabsList className="h-10 p-1 bg-slate-100 border border-slate-200/60">
            {tabItems.map((tab) => (
              <TabsTrigger 
                key={tab.id} 
                value={tab.id}
                className="px-4 py-1.5 text-xs font-medium data-[state=active]:bg-white data-[state=active]:text-slate-900 text-slate-500 rounded-md transition-all shadow-none"
              >
                {tab.label}
                {tab.count > 0 && (
                  <span className="ml-2 px-1.5 py-0.5 rounded-full bg-slate-200 text-slate-700 text-[10px] font-bold">
                    {tab.count}
                  </span>
                )}
              </TabsTrigger>
              ...
              {tab.loading ? (
                ...
                <div className="p-12 text-center bg-slate-50/10">
                  <div className="flex flex-col items-center justify-center gap-2">
                    <p className="text-sm font-medium text-slate-900">No {tab.label.toLowerCase()} found</p>
